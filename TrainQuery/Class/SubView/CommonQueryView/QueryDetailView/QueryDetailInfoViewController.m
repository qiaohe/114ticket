...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    [cell setStationStatusWithParams:_codeAndPrice];
...
- (void)cellResponder:(CustomButton*)sender
{
    ...
    switch (sender.tag) {
        case 200:{
            codeAndPrice.selectSeatType = SeatTypeWZ;
            trainOrder.seatType          = @"无座";
            trainOrder.selectTicketPrice = [codeAndPrice.wz floatValue];
            break;
        }
        case 201:{
...
- (void)analysisResponseString:(NSString*)string
{
    ...
    if ([dataSource count] == 0) {
        [[Model shareModel] showPromptBoxWithText:@"未查找到结果" modal:NO];
    }
