...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (history.startDate) {
        NSTimeInterval historydate = [[Utils dateWithString:history.startDate withFormat:@"yyyy-MM-dd"] timeIntervalSince1970];
        NSTimeInterval currentDate = [[Utils dateWithString:[Utils stringWithDate:[NSDate date] withFormat:@"yyyy-MM-dd"] withFormat:@"yyyy-MM-dd"] timeIntervalSince1970];
        if (historydate < currentDate || (historydate - currentDate) >= 20 * 24 * 60 * 60) {
            [[Model shareModel] showPromptBoxWithText:@"历史查询日期不在预售期" modal:NO];
            NSString *date = [Utils stringWithDate:[[NSDate date] dateByAddingTimeInterval:24 * 60 * 60]withFormat:@"yyyy-MM-dd"];
...
- (void)deleteQueryHistory:(CustomButton*)sender
{
    ...
    [self tableViewReload:theTableView dataSource:queryHistoryArray];
}
...
- (void)pressChooseStartCity:(UIButton*)sender
{
    ...
    if (self.delegate) {
        [self.delegate pushToViewController:cityPicker completion:nil];
    }
}
...
- (void)pressChooseStartDate:(UIButton*)sender
{
    ...
    if (self.delegate) {
        [self.delegate pushToViewController:datePicker completion:nil];
    }
}
...
- (void)pressSearchBtn:(UIButton*)sender
{
    if ([Utils textIsEmpty:self.startCity.text] || [Utils textIsEmpty:self.endCity.text] || [Utils textIsEmpty:self.startDate.text]) {
        [[Model shareModel] showPromptBoxWithText:@"搜索参数不正确" modal:YES];
    }else{
        ...
        if (self.delegate) {
            [self.delegate pushToViewController:infoView completion:^{
...
- (void)setCityViewFrame
{
    ...
    [startCity setTextColor:[self getColor:@"#3f3f3f"]];
    ...
    [endCity setTextColor:[self getColor:@"#3f3f3f"]];
...
- (void)setDateViewFrame
{
    ...
    [startDate setTextColor:[self getColor:@"#3f3f3f"]];
...
- (void)setDataViewFrame
{
    ...
    theTableView.hidden = ![queryHistoryArray count];
