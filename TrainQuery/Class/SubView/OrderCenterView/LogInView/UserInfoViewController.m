...
- (void)getUserInfo:(NSInteger)userId
{
    if (![UserDefaults shareUserDefault].getUserInfo) {
        ...
        [mobile setText:[UserDefaults shareUserDefault].mobile];
    }
}
...
- (void)parserStringFinished:(NSString *)_string request:(ASIHTTPRequest *)request
{
    ...
    if ([requestType isEqualToString:@"updateUser"]) {
        if ([[dic objectForKey:@"performStatus"] isEqualToString:@"success"]) {
            [[Model shareModel] showPromptBoxWithText:[dic objectForKey:@"performResult"] modal:NO];
            [UserDefaults shareUserDefault].getUserInfo = NO;
        }
    }
...
- (void)setSubViewFrame
{
    ...
    UIButton *mailAddressButton = [self getButtonWithFrame:CGRectMake(0, 0, userNameButton.frame.size.width, userNameButton.frame.size.height) title:@"邮箱:" textColor:[UIColor darkGrayColor] forState:UIControlStateNormal backGroundColor:[UIColor clearColor]];
    ...
    UIButton *sexButton = [self getButtonWithFrame:CGRectMake(0, 0, mailAddressButton.frame.size.width, mailAddressButton.frame.size.height) title:@"性别:" textColor:[UIColor darkGrayColor] forState:UIControlStateNormal backGroundColor:[UIColor clearColor]];
    ...
    UIButton *mobileButton = [self getButtonWithFrame:CGRectMake(0, 0, sexButton.frame.size.width, sexButton.frame.size.height) title:@"手机号码:" textColor:[UIColor darkGrayColor] forState:UIControlStateNormal backGroundColor:[UIColor clearColor]];
    [mobileButton.titleLabel setFont:[UIFont systemFontOfSize:14]];
    [mobileButton setContentHorizontalAlignment:UIControlContentHorizontalAlignmentLeft];
    [mobileButton setContentEdgeInsets:UIEdgeInsetsMake(0, 15, 0, 0)];
    [mobileButton addTarget:self action:@selector(clearKeyboard) forControlEvents:UIControlEventTouchUpInside];
    mobile = [[UITextField alloc]initWithFrame:CGRectMake(sex.frame.origin.x, sex.frame.origin.y + sex.frame.size.height - 1, sex.frame.size.width, sex.frame.size.height)];
    [mobile setBackgroundColor:[UIColor clearColor]];
    [mobile setBackground:imageNameAndType(@"userlabel", @"png")];
    mobile.leftView = mobileButton;
    mobile.leftViewMode = UITextFieldViewModeAlways;
    [mobile setContentVerticalAlignment:UIControlContentVerticalAlignmentCenter];
    [self.view addSubview:mobile];
    
    UIImageView *imageView = [[[UIImageView alloc]initWithFrame:CGRectMake(mobile.frame.origin.x, mobile.frame.origin.y + mobile.frame.size.height + 20, mobile.frame.size.width, mobile.frame.size.height)]autorelease];
...
- (void)pressUpdatePassword:(UIButton *)sender
{
    ...
    [self pushViewController:updatePasswordView completion:nil];
}
...
- (void)pressSaveButton:(UIButton *)sender
{
    ...
    NSMutableDictionary *userDic = [NSMutableDictionary dictionaryWithObjectsAndKeys:
                                   ...
                                   [Utils NULLToEmpty:mobile.text],                                 @"mobile",
...
- (void)clearKeyboard
{
    ...
    }if ([mobile canResignFirstResponder]) {
